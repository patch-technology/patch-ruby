...
module Patch
  class UpdateOffsetRequest
    ...
    def self.method_missing(message, *args, &block)
      if Object.const_defined?('Patch::UpdateOffsetRequestsApi::OPERATIONS') && Patch::UpdateOffsetRequestsApi::OPERATIONS.include?(message)
        Patch::UpdateOffsetRequestsApi.new.send(message, *args)
      else
        super
      end
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Patch::UpdateOffsetRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Patch::UpdateOffsetRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [mass_gprice_cents_usdvintage_yearserial_number].hash
    end
